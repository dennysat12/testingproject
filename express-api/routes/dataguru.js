...
router.get('/get/matpelguru/(:noinduk)', async (req, res) =>{
    let no = req.params.noinduk
    let data =await  knex.select('*').from('pengajar_materi').innerJoin('jam_akademik_a','pengajar_materi.idmateri','jam_akademik_a.id_mapel').innerJoin('kelas','pengajar_materi.idkelas','kelas.idkelas').where('pengajar_materi.nomor_induk_guru',no).orderBy('kelas.namakelas','asc').orderBy('jam_akademik_a.mapel','asc')
    console.log(data)
             res.json({
                   status: true,
                   message: 'data tampil',
